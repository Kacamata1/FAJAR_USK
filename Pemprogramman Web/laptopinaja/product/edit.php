...
<head>
    ...
    <title>LaptopInAja | Edit Laptop All In Here</title>
...
<body>
    ...
    <?php
            ...
            $id = $_GET['id'];
            $data = mysqli_query($koneksi,"select * from laptop_tb where id_laptop ='$id'");
            while($d = mysqli_fetch_array($data)){
        ?>

        <div class="section-title">
            Update Product <?php echo $d['nama_laptop'] ?>
        </div>
        <div class="container">
            <form action="" method="post" enctype="multipart/form-data">
                <label for="name">Nama Laptop</label>
                <input type="text" id="name" name="name" class="form-control" value="<?php echo $d['nama_laptop'] ?>"
                    placeholder="Nama Laptop">

                <label for="harga">Harga Laptop</label>
                <input type="number" id="harga" name="price" class="form-control"
                    value="<?php echo $d['harga_laptop'] ?>" placeholder="Harga Laptop">
                ...
                <textarea name="spesifikasi" class="form-control" maxlength="50" id="spesifikasi" cols="30"
                    rows="5"><?php echo $d['spesifikasi_laptop'] ?></textarea>
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($namaFile != "") {
                ...
                $query = mysqli_query($koneksi, "UPDATE laptop_tb SET nama_laptop='$nama', harga_laptop='$harga' , spesifikasi_laptop='$spesifikasi', gambar_laptop='$namaFile' WHERE id_laptop='$id'");
            }else {
                $query = mysqli_query($koneksi, "UPDATE laptop_tb SET nama_laptop='$nama', harga_laptop='$harga' , spesifikasi_laptop='$spesifikasi' WHERE id_laptop='$id'");
            }
